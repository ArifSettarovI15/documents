<form class="mt-lg-5 h-100"
   @isset($info->client_id)
    ...
    <fieldset class="mb-5">
        ...
        @else
            <legend>Создание нового клиента - Общество с ограниченной ответственностью</legend>
        @endisset
    </fieldset>
    <div class="form-row">
        <div class="form-group col-md-4">
            <label for="client_name" class="required">Название контрагента</label>
            <input type="hidden" name="client_type" value="ooo">
            <input type="text" name="client_name" class="form-control" id="client_name"
                   placeholder="ООО Название" required
                   value="@isset($info->client_name){{$info->client_name}}@endisset">
        </div>
        <div class="form-group col-md-4">
            <label for="client_director" class="required">Генеральный директор</label>
        ...
        <div class="form-group col-md-4">
            ...
            <input type="hidden" name="client_email" class="form-control" id="client_email">
            <button type="button" class="form-control btn btn-info" data-toggle="modal"
                    data-target="#exampleModalCenterOOO">
                Открыть список
            </button>

            <!-- Modal -->
            <div class="modal fade" id="exampleModalCenterOOO" tabindex="-1" role="dialog"
                 aria-labelledby="exampleModalCenterTitleOOO" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document" style="min-width: 1000px">
                    <div class="modal-content" style="min-width: 1000px">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalCenterTitleOOO">Список электронных адресов</h5>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="text-center">
                                <button type="button" data-target="emails_table"
                                        class="btn btn-outline-dark tooltip_wrapper emails_add_row"
                                        data-toggle="tooltip" data-placement="right" title="Tooltip on right">Добавить
                                    строку
                                </button>
                            </div>
                        </div>
    ...
    <div class="form-row">
        <div class="form-group col-md-3">
            ...
            <input type="text" name="client_inn" class="form-control" id="client_inn" placeholder="3015999999"
                   required value="@isset($info->client_inn){{$info->client_inn}}@endisset">
        </div>
        <div class="form-group col-md-3">
            <label for="client_kpp">КПП</label>
            <input type="text" name="client_kpp" class="form-control" id="client_kpp" placeholder="301511111"
                   required value="@isset($info->client_kpp){{$info->client_kpp}}@endisset">
        </div>
        <div class="form-group col-md-3">
            <label for="client_ogrn">ОГРН</label>
            <input type="text" name="client_ogrn" class="form-control" id="client_ogrn" placeholder="1133111111111"
                   required value="@isset($info->client_ogrn){{$info->client_ogrn}}@endisset">
        </div>

        <div class="form-group col-md-3">
            <label for="client_okpo">ОКПО</label>
            <input type="text" name="client_okpo" class="form-control" id="client_okpo" placeholder="10111111"
        ...
        <div class="form-group col-md-5">
            ...
            <select name="client_bank" class="form-control" id="client_bank">
                <option value="ПАО «РНКБ»"
                        @if(isset($info) and $info->client_bank == "ПАО «РНКБ»") selected {{ $in_select=true }} @endif>
                    ПАО «РНКБ»
                </option>
                <option value="АО «Генбанк»"
                        @if(isset($info) and $info->client_bank == "АО «Генбанк»") selected {{ $in_select=true }} @endif>
                    АО «Генбанк»
                </option>
                ...
                <option value="ОАО «Банк ЧБРР»"
                        @if(isset($info) and $info->client_bank == "ОАО «Банк ЧБРР»") selected {{ $in_select=true }} @endif>
                    ОАО «Банк ЧБРР»
                </option>
                <option value="АО «АБ «РОССИЯ»"
                        @if(isset($info) and $info->client_bank == "АО «АБ «РОССИЯ»") selected {{ $in_select=true }} @endif>
                    АО «АБ «РОССИЯ»
                </option>
                <option value="АО КБ «ИС Банк»"
                        @if(isset($info) and $info->client_bank == "АО КБ «ИС Банк»") selected {{ $in_select=true }} @endif>
                    АО КБ «ИС Банк»
                </option>
    ...
    <div class="form-row">
            ...
            <div class="form-group mt-3 col-md-6">
                ...
                <div class="ml-5">
                    @foreach($info->client_contracts as $contract)
                        <a href="{{route('manager.contracts.show', $contract->contract_id)}}">Договор
                            № {{$contract->contract_number}} от {{$contract->contract_date}}</a>
                        <br>
                    @endforeach
                </div>
            </div>
        ...
        @if(isset($info) and $info->client_has_invoices)
            <div class="form-group col-md-6">
                <h3>Счета клиента:</h3>
                <div class="ml-5">
                    <a href="{{route('manager.invoices.client', $info->client_id)}}">Список
                        счетов {{$info->client_name}}</a>
                </div>
            </div>
        @endif
    </div>
