...
__webpack_require__(/*! ./jquery.toast.min */ "./resources/js/jquery.toast.min.js");

__webpack_require__(/*! ./bootstrap-datepicker.min */ "./resources/js/bootstrap-datepicker.min.js");

__webpack_require__(/*! ./bootstrap-datepicker.ru.min */ "./resources/js/bootstrap-datepicker.ru.min.js");
...
$(document).on('change', '.js_status_change', function () {
  ...
  var input = $(this);
  if (options['value'] === 1) options['value'] = 0;else options['value'] = 1;
...
function check_rows() {
  ...
  if (rows >= services) {
    $('.add_contract_service_button_wrap').css({
      'display': 'none'
    });
  } else {
    $('.add_contract_service_button_wrap').css({
...
$(document).on("click", '#modal-btn-si', function () {
  ...
  $.ajax({
    ...
    success: function success(response) {
      ...
      if (response.status === true || typeof response.status === 'undefined') $(item).closest('tr').remove();
    }
  });
});
...
$(document).ready(function () {
  if ($('div').hasClass('services_status_bars')) {
    $('.smtp_status_true').css({
      'display': 'none'
    });
    $('.smtp_status_false').css({
      'display': 'none'
    });
    $('.converter_status_true').css({
      'display': 'none'
    });
    $('.converter_status_warning').css({
      'display': 'none'
    });
    $('.converter_status_false').css({
      'display': 'none'
    });
  ...
  $.ajax({
    ...
    success: function success(response) {
      ...
      if (current !== response.status.toString()) {
        $('.' + response.target + current).css({
          'display': 'none'
        });
        $('.' + response.target + response.status.toString()).css({
          'display': 'block'
        });
...
var debouncer = debounce(function (data) {
  send_filter(data);
}, 100);
...
$(document).on('click', '.get_invoice_for_client', function () {
  ...
  if (typeof contract === 'undefined') {
    ...
    show_message({
      'status': true,
      'message': 'Формируем файл счета, ожидайте'
    });
  }
});
...
$(document).on('click', '.button_delete_services', function () {
  console.log(this);
  if ($(this).closest('form').find('.services_data_wrapper').length > 1) $(this).closest('.services_data_wrapper').remove();
